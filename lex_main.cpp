...
bool IsOpr(char *s) {
    for (int i = 0; i < 41; i++) {
        if (strcmp(operatorOrDelimiter[i], s) == 0)
            return 1;
    }
    return 0;
}
        ...
        if (IsLetter(resourceProject[pProject])) {
            ...
            if (resourceProject[pProject] == '0' &&
                ...
                if (resourceProject[pProject + 1] == 'x' ||  // hex
                    ...
                    if (!IsOpr(_) && resourceProject[pProject] != ' ') {
                        throwError("Hex Error", lineMap[pProject],
                                   colMap[pProject]);
                    }
                    ...
                    while (IsDigit(resourceProject[pProject])) {
                        if (!IsBinOK(resourceProject[pProject])) {
                            char msg[1004];
                            throwError("Bin Error", lineMap[pProject],
                                       colMap[pProject]);
                            throw msg;
                        }
                        token[count++] = resourceProject[pProject], pProject++;
                ...
                while (IsDigit(resourceProject[pProject])) {
                    if (!IsOctOK(resourceProject[pProject])) {
                        char msg[1004];
                        throwError("Oct Error", lineMap[pProject],
                                   colMap[pProject]);
                        throw msg;
                    }
                    token[count++] = resourceProject[pProject], pProject++;
                ...
                if ((!IsOpr(_) || _[0] == '.') &&
                    ...
                    char msg[1004];
                    throwError("Dec Error", lineMap[pProject],
                               colMap[pProject]);
                    throw msg;
                }
            }
            ...
            while (ch != (resourceProject[pProject])) {  // dec
                ...
                if (pProject == resourceLen) {
                    char msg[1004];
                    throwError("String/Char Match Error",
                               lineMap[pProject - count],
...
int main() {
    ...
    while (syn != 0) {
        ...
        if (syn == 100) {
            ...
            if (!ok)
                strcpy(IDentifierTbl[IDentifierTblLen++], token);
